...
def main():
    ...
    parser.add_argument('--inputDir','-i', type=str,metavar="FILE/FOLDER", help='Required, Path to the input file or directory', required=True)
    parser.add_argument('--outputDir','-o', type=str,metavar="FOLDER", help='Required, Path to the output directory', required=True)
    parser.add_argument('--pattern',metavar="CHAR", type=str, help="Required, Telomere repeat sequence (in 5' to 3' orientation). For e.g., in human, it's recommended to use CCCTAA", required=True)
    parser.add_argument('--minSeqLength',metavar="INT", type=int, help='Minimum length required for long read, default = 9kbp', default=9000)
    parser.add_argument('--rawcountpattern', action='store_true', help='Output raw count of pattern abundance of each window')
    parser.add_argument('--telophrase', nargs='+', metavar="INT",type=int, help=' k-mer of telomere pattern, by default, telomere pattern length minus 2')
    parser.add_argument('--cutoff',nargs='+',metavar="FLOAT", type=float, help='Threshold of TRC to be telomere, can be 0.4, 0.5,... ', default=0.7)
    parser.add_argument('--windowSize',metavar="INT", type=int, help='Sliding window size', default=100)
    parser.add_argument('--slide',metavar="INT", type=int, help=' Window sliding step, default is initial telomere length')
    parser.add_argument('--trimfirst',metavar="INT", type=int, help='Trimming off first number of base pair to prevent adapter', default=100)
    parser.add_argument('--maxlengthtelo',metavar="INT", type=int, help='Longest value can be for telomere', default=20000)
    parser.add_argument('--plot', action='store_true', help='Plot of changes in mean window and change point detected, boolean')
    parser.add_argument('--rangecp',metavar="INT", type=int, help='optional, set range of changepoint plot for visualization, default is maxlengthtelo')
    parser.add_argument('--read_check',metavar="STR", type=str, help='optional, get telomere of a specific read')
    parser.add_argument('--override','-ov', action='store_true', help='Override telolengths_all.csv file but keep subset fastq')
    parser.add_argument('--threads','-t',metavar="INT", type=int, help='Number of CPU cores to use (by default, all available cores)', default=None)
